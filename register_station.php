...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($latitud) || empty($longitud) || empty($descripcion)) {
        ...
        if ($exists > 0) {
            ...
            $sql = "INSERT INTO est (Latitud, Longitud, Descr) VALUES (:latitud, :longitud, :descripcion)";
...
<script>
    const map = L.map('map').setView([-17.3939, -66.1577], 6); // Coordenadas de Bolivia como punto de referencia
    ...
    function updateMap(lat, lon) {
        if (marker) {
            map.removeLayer(marker); // Eliminar el marcador existente
        }
        marker = L.marker([lat, lon]).addTo(map); // Agregar un nuevo marcador
        map.setView([lat, lon], 13); // Centrar el mapa en el nuevo marcador
    }

    // Manejar cambios en los campos de latitud y longitud
